---
{
  "title": "Pikeman",
  "description": "Pikeman: base values, rules and related units in Unciv.",
  "gameVersion": "4.21.20",
  "appBuild": 1293,
  "ruleset": "Civ V - Gods & Kings",
  "sources": [
    "snapshot:4.21.20-1293"
  ],
  "lastUpdated": "2026-09-12"
}
---

<span class="database-icon-frame"><img class="database-icon" src="/Unciv-Wiki/game-assets/unit/pikeman.png" alt="Pikeman" width="80" height="80" loading="lazy" decoding="async" /></span>

[Icon sources and credits](/Unciv-Wiki/credits/)

**Gods & Kings ruleset base values** are not final prices in every game. Speed, difficulty, city count and rule effects can modify costs. [How costs work](/Unciv-Wiki/mechanics/).

| Attribute | Value |
| --- | --- |
| Type | Sword |
| Movement | 2 |
| Strength | 16 |
| Base cost | 90 |
| Required technology | [Civil Service](/Unciv-Wiki/database/technologies/civil-service/) |
| Obsolete technology | [Metallurgy](/Unciv-Wiki/database/technologies/metallurgy/) |
| Upgrades to | [Lancer](/Unciv-Wiki/database/units/lancer/) |

## Rules and effects

- [+50]% Strength &lt;vs [Mounted] units&gt;

## Unlocks and related entries

- [Spearman](/Unciv-Wiki/database/units/spearman/)
- [Hoplite](/Unciv-Wiki/database/units/hoplite/)
- [Persian Immortal](/Unciv-Wiki/database/units/persian-immortal/)
- [Pictish Warrior](/Unciv-Wiki/database/units/pictish-warrior/)
- [Marauder](/Unciv-Wiki/database/units/marauder/)
- [Landsknecht](/Unciv-Wiki/database/units/landsknecht/)

