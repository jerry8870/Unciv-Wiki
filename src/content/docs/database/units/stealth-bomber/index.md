---
{
  "title": "Stealth Bomber",
  "description": "Stealth Bomber: base values, rules and related units in Unciv.",
  "gameVersion": "4.21.20",
  "appBuild": 1293,
  "ruleset": "Civ V - Gods & Kings",
  "sources": [
    "snapshot:4.21.20-1293"
  ],
  "lastUpdated": "2026-09-12"
}
---

<span class="database-icon-frame"><img class="database-icon" src="/Unciv-Wiki/game-assets/unit/stealth-bomber.png" alt="Stealth Bomber" width="80" height="80" loading="lazy" decoding="async" /></span>

[Icon sources and credits](/Unciv-Wiki/credits/)

**Gods & Kings ruleset base values** are not final prices in every game. Speed, difficulty, city count and rule effects can modify costs. [How costs work](/Unciv-Wiki/mechanics/).

| Attribute | Value |
| --- | --- |
| Type | Bomber |
| Movement | 2 |
| Strength | 85 |
| Ranged strength | 85 |
| Range | 20 |
| Base cost | 425 |
| Required technology | [Stealth](/Unciv-Wiki/database/technologies/stealth/) |
| Required resource | Aluminum |

## Rules and effects

- Damage taken from interception reduced by [100]%
- Cannot be carried by [Carrier] units
- [+4] Sight
- Can see over obstacles

## Promotions

- Evasion

## Unlocks and related entries

- [Bomber](/Unciv-Wiki/database/units/bomber/)
- [B17](/Unciv-Wiki/database/units/b17/)

