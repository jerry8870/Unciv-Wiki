---
{
  "title": "Bronze Working",
  "description": "Bronze Working: base values, rules and related technologies in Unciv.",
  "gameVersion": "4.21.20",
  "appBuild": 1293,
  "ruleset": "Civ V - Gods & Kings",
  "sources": [
    "snapshot:4.21.20-1293"
  ],
  "lastUpdated": "2026-09-12"
}
---

<span class="database-icon-frame"><img class="database-icon" src="/Unciv-Wiki/game-assets/technology/bronze-working.png" alt="Bronze Working" width="80" height="80" loading="lazy" decoding="async" /></span>

[Icon sources and credits](/Unciv-Wiki/credits/)

**Gods & Kings ruleset base values** are not final prices in every game. Speed, difficulty, city count and rule effects can modify costs. [How costs work](/Unciv-Wiki/mechanics/).

| Attribute | Value |
| --- | --- |
| Base cost | 55 |
| Era | Ancient era |

Inherited from the required technology column.

Prerequisites: [Mining](/Unciv-Wiki/database/technologies/mining/)

## Rules and effects

- [+50]% weight to this choice for AI decisions

## Unlocks and related entries

- [Spearman](/Unciv-Wiki/database/units/spearman/)
- [Hoplite](/Unciv-Wiki/database/units/hoplite/)
- [Persian Immortal](/Unciv-Wiki/database/units/persian-immortal/)
- [Battering Ram](/Unciv-Wiki/database/units/battering-ram/)
- [Pictish Warrior](/Unciv-Wiki/database/units/pictish-warrior/)
- [Marauder](/Unciv-Wiki/database/units/marauder/)
- [Barracks](/Unciv-Wiki/database/buildings/barracks/)
- [Krepost](/Unciv-Wiki/database/buildings/krepost/)
- [Statue of Zeus](/Unciv-Wiki/database/buildings/statue-of-zeus/)
- [Iron Working](/Unciv-Wiki/database/technologies/iron-working/)

> &#x27;Here Hector entered, with a spear eleven cubits long in his hand; the bronze point gleamed in front of him, and was fastened to the shaft of the spear by a ring of gold.&#x27; - Homer

